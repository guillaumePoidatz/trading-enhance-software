...
from RLTraining.envs.data_loaders import DiskDataLoader
from RLTraining.envs.statistics_recorder import StatisticsRecorder
from RLTraining.envs.gym_space_builder import GymSpaceBuilderLong
from RLTraining.envs.scaler import Scaler
...
class LearningCryptoEnv(gym.Env):
    def __init__(
        ...
        **env_config
        ...
        self.mainDirectoryName = env_config.get("logdir", "")
        ...
        self.price_array, self.tech_array_total = DiskDataLoader(
            directoryName = self.mainDirectoryName,
            dataset_name=self.dataset_name).load_dataset()
        ...
        self.observation_space, self.action_space = GymSpaceBuilderLong(observation_dim=self.observation_dim).get_spaces()
    ...
    def _reset_env_state(self):
        ...
        if self.regime == "training":
            ...
            random_interval = 0
            self.max_step = self.test_end[random_interval] - self.test_start[random_interval] - 1
            self.time_absolute = self.test_start[random_interval]
